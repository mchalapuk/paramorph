...
function parse(yaml) {
    ...
    Object.keys(config.collections)
        ...
        var collection = config.collections[name];
    ...
    offensive_1.default(config.image, 'config.image').is.either.Empty.or.aString();
    offensive_1.default(config.baseUrl, 'config.baseUrl').is.aString();
    offensive_1.default(config.locale, 'config.locale').is.aString();
    offensive_1.default(config.menu, 'config.menu').is.anArray();
    config.menu.forEach(function (entry, i) {
        offensive_1.default(entry.title, "config.menu[" + i + "].title").is.aString();
        offensive_1.default(entry.short, "config.menu[" + i + "].short").is.aString();
        offensive_1.default(entry.url, "config.menu[" + i + "].url").is.aString();
        offensive_1.default(entry.icon, "config.menu[" + i + "].icon").is.either.Empty.or.aString();
    });
    return config;
}
exports.parse = parse;
