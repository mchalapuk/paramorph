...
function descriptionFromContent(page) {
    ...
    return entities.decode(utils_1.stripTags(server_1.renderToStaticMarkup(router)));
}
function descriptionFromPages(page) {
    return entities.decode(index.title + " " + page.title + ": " + page.pages.map(function (p) { return p.title; }).join(', '));
}
