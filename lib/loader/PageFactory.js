...
var PageFactory = /** @class */ (function () {
    ...
    PageFactory.prototype.create0 = function (PageType, file, collection, matter) {
        ...
        var url = matter.permalink || defaultUrl(title);
        return new PageType(url, title, matter.description || '', matter.image || null, collection, matter.layout || '', file.path, matter.output || true, matter.feed || true, categories, matter.tags || [], new Date(matter.date).getTime());
    };
    return PageFactory;
}());
